...
#include<windows.h>
using namespace std;
int BackGroundColor=0xf0,TextColor=0xa;
//--------------API area---------------------------
class PrintStream{
...
class liquid{
	public:
		...
		liquid(string id,int p){
			ID=id,price=p;cannot_be_sell=0;
		}
		liquid()=default;
};
...
void cf2f(string s){
	...
	print<<'\n'<<"MaterialLiquid"<<'\n';
	for(auto a:CraftingList[s].ml)print<<a.first<<"*"<<a.second<<"mB";
	print<<'\n'<<"Result"<<'\n';
	for(auto a:CraftingList[s].as)print<<a.second<<"x"<<a.first;
	for(auto a:CraftingList[s].al)print<<a.first<<"*"<<a.second<<"mB";
	print<<'\n';
}
...
void CreateModObjects(string name){
	...
	print<<"forbidding"<<'\n';
	fin>>n;//forbidden_stuff
	while(n--){
		fin>>s;
		StuffTypes[s].cannot_be_sell=1;
	}
	fin>>n;//forbidden_liquid
	while(n--){
		fin>>s;
		LqdTypes[s].cannot_be_sell=1;
	}
}
void GetConfig(string path){
	FILE* conf=fopen(path.c_str(),"r");
	fscanf(conf,"BackGround=%d",&BackGroundColor);
	fscanf(conf,"Text=%d",&TextColor);
}
void configread(string search){
	WIN32_FIND_DATA findFileData;
    HANDLE hFind = INVALID_HANDLE_VALUE;
    int f=1;
    std::string searchPath ="configs\\*";
    hFind = FindFirstFile(searchPath.c_str(), &findFileData);
    do {
        if (!(findFileData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)) {
        	string t="configs\\";
    ...
    FindClose(hFind);
    if(f==1){
    	ofstream fout("configs\\config.txt");
    	fout<<"BackGround=128\nText=1\n";
	}
}
void modread() {
...
class Player{
	...
	public:
		...
		void BuyLiquid(string s,int mB){
			int t=mB*LqdTypes[s].price;
			if(t>this->Money){
				cout<<"Not_enough_money";Sleep(250);return;
			}
			this->Money-=t;
			Lqd_V[s]+=mB;
		}
		...
		void SellStuff(string s,int pcs){
			int t=min(this->Stuff_Count[s],pcs);
			Stuff_Count[s]-=t;
			this->money_plus(t*StuffTypes[s].price);
		}
		void Save(){
		...
		bool operation(){
			...
			,Cols,Tnks,Item={"Collector","Tank","Stuff","Cow","Express"}
			,LoS={"Liquid","Stuff"},
			allliquids,liquids,stuffs,belong_stuffs;
			for(auto i:CltorTypes){
				if(i.second.empty())continue;
				Cols.push_back(i.first);
			}for(auto i:LqdTypes){
				allliquids.push_back(i.first);
			}
			...
			for(auto i:StuffTypes){
				...
				else belong_stuffs.push_back(i.first);
				if(!i.second.cannot_be_sell)stuffs.push_back(i.first);
			}
			...
			switch(Optid){
				...
				case 2:{
					int type=ChooseMenu(LoS,Emp,Emp,LoS.size(),"----","--->",'e','w','s','q');
					if(type==-1)break;
					if(!type){
						int t,Id;
						if(liquids.empty()){
						...
						int t,Id;
						if(belong_stuffs.empty()){
							print<<"Failed"<<'\n';
							Sleep(200);
							break;
						}
						Id=ChooseMenu(belong_stuffs,Emp,Emp,belong_stuffs.size(),"----","--->",'e','w','s','q',s2m);
						if(Id==-1)break;
						t=ChooseNumber("Stuff_Sell","pcs",'w','s','e');
						this->SellStuff(belong_stuffs[Id],t);
					}
				}
				case 3:{
					EquipC();
